...
var CHARACTER_ETHNICITY = ".chara_profile_title",
    ...
    CHARACTER_ITEMS     = ".item_detail_box",
    ...
    CHARACTER_ITEM_LINK = ".bt_db_item_detail",
    ...
    CHARACTER_SECTION   = ".ic_silver",
    CHARACTER_MINOUNT   = ".ic_reflection_box";
//end character block
...
var Lodestone = {
  ...
  getCharacterInfo: function(id, no_icons, callback) {
    ...
    createDOM(CHARACTER + id + "/", function(window, document) {
      ...
      if(items_ > 0) {
        result.averageItemLevel = Math.round(ilvl / items_);
      }
      ...
      for(var i = 0; i < silver.length; ++i) {
        try {
          var section = silver[i].textContent.trim();
          if(section == "Minions" || section == "Mounts") {
            var children = silver[i].parentNode.querySelectorAll(CHARACTER_MINOUNT);
